...
Verifies that inserts, deletes and updates are processed in JDBC batches.
...
You can decide to batch all inserts, updates, delete. Prior to Hibernate 5.2, batching, when enabled, was applied to all inserts, updates and deletes ([Hibernate 5.2 it is also possible to enable bathcing only for a given session](https://docs.jboss.org/hibernate/orm/5.2/userguide/html_single/chapters/batch/Batching.html)).
