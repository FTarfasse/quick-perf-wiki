...
If you may want to progressively fix these failing tests. To do this, you could annotate the failing tests with @DisableGlobalAnnotations with a reason, for example @DisableGlobalAnnotations(reason="Investigate why global annotations are failing). After that, you could progressively remove the annotation and see why the tests are failing.
